...
-- Una persona no puede tener dos entradas activas para el mismo servicio, día
-- y especialista. `nulls not distinct` hace que "cualquier especialista"
-- (staff_id NULL) también cuente como duplicado. Parcial sobre notified_at:
-- una vez avisada, puede volver a anotarse.
create unique index waitlist_sin_duplicados
  on waitlist_entries (business_id, service_id, date, customer_phone, staff_id)
  nulls not distinct
  where notified_at is null;

-- Límites de uso de los endpoints públicos. Cada fila es "esta llave usó este
-- recurso en este instante"; el tope se cuenta en una ventana de tiempo, igual
-- que ya lo hacía login_tokens. Sin esto, /api/waitlist y /api/appointments
-- son una manguera de gasto en mensajes de WhatsApp.
